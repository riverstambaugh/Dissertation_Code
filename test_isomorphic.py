...
#assert check_isomorphic(d, e) == brute_force_checker(d, e) == check_isomorphic_graphwise(d, e)
#
##Not isomorphic because the class shapes differ
#assert check_isomorphic(f, g) == brute_force_checker(f, g) == check_isomorphic_graphwise(f, g)
#
##Not isomorphic because canonical word forms differ
#assert check_isomorphic(h, i) == brute_force_checker(h, i) == check_isomorphic_graphwise(h, i)
#
##Not isomorphic because generator minimal alphabet sizes differ
#assert check_isomorphic(j, k) == brute_force_checker(j, k) == check_isomorphic_graphwise(j, k)
#
##Not isomorphic because one has more unused generators than the other
#assert check_isomorphic(l, m) == brute_force_checker(l, m) == check_isomorphic_graphwise(l, m)
#
##Not isomorphic because O has more rules than N
#assert check_isomorphic(o, n) == brute_force_checker(o, n) == check_isomorphic_graphwise(o, n)
#
##Not isomorphic because P has a different small overlap class size than Q
#assert check_isomorphic(p, q) == brute_force_checker(p, q) == check_isomorphic_graphwise(p, q)
#
##Not isomorphic because b (in R) must map to e, but also to itself.
#assert check_isomorphic(r, s) == brute_force_checker(r, s) == check_isomorphic_graphwise(r, s)
#
##Isomorphic, each alphabet letter in T maps to the next alphabet letter in U
#presentation.change_alphabet(u, '104729fp')
#assert check_isomorphic(t, u) == brute_force_checker(t, u) == check_isomorphic_graphwise(t, u)
#
##Presentations v and w are isomorphic and have multiple redundant elements. 
#assert check_isomorphic(w, v) == brute_force_checker(w, v) == check_isomorphic_graphwise(w, v)
...
#assert brute_force_checker(vi, wi) == check_isomorphic(vi, wi) == check_isomorphic_graphwise(vi, wi)
...
#This example works!
assert check_isomorphic(noalph1, noalph2) == check_isomorphic_graphwise(noalph1, noalph2) == brute_force_checker(noalph1, noalph2)
...
assert check_isomorphic(noalph1, noalph3) == check_isomorphic_graphwise(noalph1, noalph3) == brute_force_checker(noalph1, noalph3)
...
assert check_isomorphic(empty1, empty2) == check_isomorphic_graphwise(empty1, empty2) == brute_force_checker(empty1, empty2)

#All algorithms will also return false if one presentation is empty and the other is nonempty.
assert check_isomorphic(empty1, a) == check_isomorphic_graphwise(empty1, a) == brute_force_checker(empty1, a)
assert check_isomorphic(b, empty2) == check_isomorphic_graphwise(b, empty2) == brute_force_checker(b, empty2)
...
assert brute_force_checker(smallrl, smallrl2) == check_isomorphic(smallrl, smallrl2) == check_isomorphic_graphwise(smallrl, smallrl2)
...
assert brute_force_checker(tst1, tst2) == check_isomorphic(tst1, tst2) == check_isomorphic_graphwise(tst1, tst2)
